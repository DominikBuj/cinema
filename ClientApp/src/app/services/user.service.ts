...
export class UserService {
  ...
  signIn(params: any): Observable<User> {
    return this.http.post<User>(`${this.baseUrl}users/sign-in`, params)
      .pipe(map(user => {
  ...
  signUp(params: any): Observable<User> {
    return this.http.post<User>(`${this.baseUrl}users/sign-up`, params)
      .pipe(map(user => {
        localStorage.setItem('user', JSON.stringify(user));
        this.user.next(user);
        return user;
  ...
  updateUser(id: number, params: any): Observable<User> {
    return this.http.patch<User>(`${this.baseUrl}users/${id}`, params)
      .pipe(map(user => {
